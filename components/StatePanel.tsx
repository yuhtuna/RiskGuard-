...
const KEY_META: Record<string, { label: string; icon: React.ReactNode }> = {
    ...
    sast_report: { label: 'SAST Report', icon: <Icon path="M3 21v-4m0 0V5a2 2 0 012-2h6.5l1 1H21l-3 6 3 6h-8.5l-1-1H5a2 2 0 00-2 2zm9-13.5V9" /> },
    ...
    dast_report: { label: 'DAST Report', icon: <Icon path="M13 10V3L4 14h7v7l9-11h-7z" /> },
...
const StatePanel: React.FC<StatePanelProps> = ({ graphState }) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg p-4 flex-grow flex flex-col min-h-[250px]">
            <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-600 pb-2 mb-2">Graph State</h2>
            <div ref={scrollContainerRef} className="flex-grow basis-0 overflow-auto bg-slate-50 dark:bg-gray-900 rounded-md p-1.5 space-y-0.5">
                {Object.keys(graphState).length > 0 ? (
                    orderedKeys.map(key => {
